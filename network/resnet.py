...
class ResNet_V1():
    def __init__(self, filters, depths, conv_weights, is_training):
        ...
        self.conv_weights = conv_weights
        self.is_training = is_training
    def resnet_layer(self, inputs, scope, num_filters=16, kernel_size=3, strides=1, bn = True, relu = True, conv_first=True):
        x = inputs
        if conv_first:
            x = conv_layer(x, self.conv_weights, filter=num_filters, kernel=[kernel_size,kernel_size], stride=strides, layer_name=scope+'_conv')
            if bn:
    ...
    def forward(self, input_x):
        with tf.variable_scope('extractor'):
            if (self.depths - 2) % 6 != 0:
                raise ValueError('depth should be 6n+2.')
            num_res_blocks = int((self.depths-2)/6)
            num_filters = self.filters
            x = self.resnet_layer(inputs=input_x, scope='conv0', num_filters=num_filters)
            for stage in range(3):
                for res_block in range(num_res_blocks):
                    strides = 1
                    ...
                    y = self.resnet_layer(inputs=y,
                                          scope='stage_%d_block_%d_layer_2' % (stage, res_block), 
                                          num_filters=num_filters, relu=False)
                    if stage > 0 and res_block == 0:
                        x = self.resnet_layer(inputs=x, scope = 'stage_%d_block_%d_layer_3' % (stage, res_block),
                                              num_filters=num_filters, kernel_size=1, 
                                              strides=strides, relu=False, bn=False)
                    x = x + y
                    if not (stage == 2 and res_block == (num_res_blocks-1)):
                        x = Relu(x)
                num_filters *= 2
            x = Batch_Normalization(x, training=self.is_training, scope='final')
            global_pool = tf.reduce_mean(x, [1, 2])
